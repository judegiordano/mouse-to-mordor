...
use crate::{
    app_config::{AppConfig, DistanceTraveled, Progress, APP_NAME, CONFIG_NAME},
    constants::{FEET_IN_MILES, INCHES_IN_FEET, INCHES_IN_PIXELS},
    state::AppState,
};

pub async fn run(window: tauri::Window, state: State<'_, Arc<AppState>>) -> Result<(), String> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            // skip formatting for this branch cuz tokio macros breaks spacing :/
            #[rustfmt::skip]
            tokio::select! {
                _ = interval.tick() => {
